...
import {fetchWithRetry, stringToBase64} from '../utils/utils';
...
export class AnalysisService {
  ...
  private createWorker(): Worker {
    const workerCode = `
      ...
      const { value: textContent } = await self.mammoth.extractRawText({ arrayBuffer });
            ...
            fileName.endsWith('.xml') || mimeType === 'application/xml' || mimeType === 'text/xml'
          ) {
  ...
  private createHtmlPreview(text: string, title: string): string {
    ...
    `;
    // Encode using TextEncoder to correctly handle Unicode characters.
    const base64 = stringToBase64(htmlContent);
